...
my $MAX_ASY=$opt_s ? $opt_s : 6; # maximal asymmetry of miRNA/miRNA* duplex
my $FLANK=$opt_f ? $opt_f : 10;
my $help=$opt_h ? 1 : 0;

# constant
my $DOMINANCE=0.6; ## 0.9 0715
my $MATURE_SIZE=21; # canonical size of an miRNA mature sequence
...
my @chrName = sort keys %refId_tags ;
my $c=0;
foreach ( sort keys %refId_tags ) {  #sort keys %refId_tags
	...
	print LOG "\t$refId:$strand breakpoints: break_n\n";


	# my @res =$break_indexes{"9079723"};
	# print "One Res:========== $res[0] ==========\n"; 
...
#	print LOG "\t$refId:$strand csites_repeat: $csites_repeat_n csites_remain: $csites_remain_n\n";
	...
	# for (0..$#csites){
	#    push @array,$_ if $csites[$_-1] =~ /^51342991$/;
	# }
	# if (@array==undef){
	#    print "undef\n";
	# }
	# else{
	#    $res =$array[0];
	#    print "One Res: $res\t$csites[$res-1]\n";
	# }
	# foreach my $i (0..100) { # 0..$#csites
	# 	my $csite=$csites[$i];
	# 	print "\t$i\t$csite\n";
	# 	}
	foreach my $i (0..$#csites) { #   
		...
		### transform coors
		# print "$dsfold->{reason}\t$usfold->{reason}\n";
		if ($dsfold) {
			...
			my ($beg,$end);	
			if ($locates[3] eq "+") {
				$beg=$locates[1]+$beg_local;
				$end=$locates[1]+$end_local;
			}
			else {
				$beg=$locates[2]-$end_local+2;
				$end=$locates[2]-$beg_local+2;
			}
		...
		foreach my $fold ($dsfold,$usfold) {
			...
			foreach my $i (@indexes) {
				...
				##my $tmp=[$tagId,$beg_local-1,$end_local,$tag->[3],$tag_count,$tag_seq];
				push @{$fold->{tags}},$tmp;
				#print "$tag->[2],$tag->[1], $beg,$end,$strand,$tagId,$beg_local-1,$end_local,$tag->[3],$tag_count,$tag_seq\n";
			}
		}
...
#			print Dumper($fold_pf),"\n";
			...
			$csite_pass{$csite}=1;
			my $csite_next=$csites[$i+1];
			if ($csite_next < $fold_pf->{end} && $csite_next > $fold_pf->{beg}) {
				$csite_pass{$csite_next}=1;
			}
			my $csite_front=$csites[$i-1];
			if ($csite_front < $fold_pf->{end} && $csite_front > $fold_pf->{beg}) {
				$csite_pass{$csite_front}=1;
			}
		}
	}
	my $csites_pass_n=scalar keys %csite_pass;
	print LOG "\t$refId:$strand $break_n $csites_n($csites_repeat_n) $csites_pass_n\n";
	# print "\t$refId:$strand $break_n $csites_n($csites_repeat_n) $csites_pass_n\n";
}
...
sub mkgff {
	...
	my $m5Id=$seqId."-5p";
	my $m3Id=$seqId."-3p";
	my $m5_beg=index($seq,$m5,0);
	my $m5_end=$m5_beg+length($m5)-1;
	my $m3_beg=index($seq,$m3,0);
	my $m3_end=$m3_beg+length($m3)-1;
	...
	if ($strand eq "+") {
		$m5_beg_abs=$beg+$m5_beg;
		$m5_end_abs=$beg+$m5_end;
		$m3_beg_abs=$beg+$m3_beg;
		$m3_end_abs=$beg+$m3_end;
	}
	else {
		$m5_beg_abs=$end-$m5_end;
		$m5_end_abs=$end-$m5_beg;
		$m3_beg_abs=$end-$m3_end;
		$m3_end_abs=$end-$m3_beg;
	}
...
#	$aln.="$m3_str $m3Id $fold->{m3count}\n";
	...
	foreach my $tag (sort {$a->[1] <=> $b->[1] or $a->[2] <=> $b->[2]} @{$fold->{tags}}) {
		...
		# print "$seq_len,$tag_beg,$tag_end-$tag_beg\n$tag_seq\n$str\n";
		substr($str,$tag_beg,$tag_end-$tag_beg)=$tag_seq;
...
sub subseq{
	...
	# my $subseq=substr($seqId_seq->{$seqId},$beg-1,$end-$beg+1); tags
	my $subseq=substr($seqId_seq->{$seqId},$beg,$end-$beg);
	if ($strand eq "-") {
...
Usage: mireap.pl -i <smrna.fa> -m <map.txt> -r <reference.fa> -o <outdir>
  ...
  -s <int>   Maximal asymmetry of miRNA/miRNA* duplex (5)
